...
def get_args():
    ...
    parser.add_argument('--root_path', type=str, default='../Datasets/APTOS/APTOS_images/train_images')
    parser.add_argument('--csv_file_path', type=str, default='../CSVs/')
    parser.add_argument("--logdir", type=str, required=False, default="./logs/aptos/", help="Log directory path")
    ...
    parser.add_argument('--n_distill', type=int, default=20, help='start to use the kld loss')
    ...
    parser.add_argument('--rel_weight', type=float, default=25, help='whether use the CCD loss')
    ...
    parser.add_argument('--n_data', type=int, default=6400, help='total number of training samples.')
    parser.add_argument('--t_decay', type=float,  default=0.99, help='ema_decay')
    ...
    parser.add_argument('--scheduler', type=str,  default='OneCycleLR', help='sch_str')
...
if __name__ == "__main__":
    ...
    test_dl = DataLoader(test_ds, batch_size=args.batch_size, 
                          ...
                          worker_init_fn=worker_init_fn)
    freq = get_labels_frequency(args.csv_file_path + args.dataset + '/' + args.split + '_train.csv', 'diagnosis', 'id_code')
    freq = freq.values
    weights = freq.sum() / freq
    print(weights)

    # Loading Models
    student = DenseNet121(hidden_units=args.feat_dim, out_size=n_classes, drop_rate=args.drop_rate)
    teacher = DenseNet121(hidden_units=args.feat_dim, out_size=n_classes, drop_rate=args.drop_rate)

    for param in teacher.parameters():
        param.detach_()

    # Fit the model
    fit(student, teacher, train_dl, test_dl, weights, class_index, logger, args, device=args.device)
