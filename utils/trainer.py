...
from utils.losses import KLD
from utils.STA import STA_Loss
...
def fit(
    ...
    if args.optimizer == "adam":
        optimizer = optim.Adam(filter(lambda p: p.requires_grad, student.parameters()), lr=args.lr, weight_decay=1e-5)
    ...
    if args.scheduler == 'StepLR':
        sch = optim.lr_scheduler.StepLR(optimizer, step_size=args.epochs // 3, gamma=0.1)
    elif args.scheduler == 'CosineAnnealingLR':
        sch = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx,((s_images, t_images), target, index, sample_idx) in tqdm_train_iterator:
            ...
            if epoch>=args.n_distill:
                ...
                consistency_dist = kld(t_logits, s_logits)
                consistency_loss = consistency_weight * consistency_dist

                relation_loss = STA(s_ftrs, t_ftrs, index.cuda(), target, class_index, args.nce_p, sample_idx.cuda())
        ...
        acc, pre, rec, f1 = test(student, test_dl, logger, verbose=True, device=device)
