...
class ISIC_InstanceSample(Dataset):
    ...
    def __init__(self, root_dir, csv_file, CCD_mode, transform=None, p=10, k=4096,
        ...
        self.cls_positive = [np.asarray(self.cls_positive[i]) for i in range(n_classes)]
        self.cls_negative = [np.asarray(self.cls_negative[i]) for i in range(n_classes)]
...
def load_dataset(args, p=10, k=4096, mode='exact', is_sample=True, percent=1.0):
    csv_file_train = args.csv_file_path + args.dataset + '/' + args.split + '_train.csv'
    ...
    test_set = ISIC_Dataset(root_dir=args.root_path, csv_file=csv_file_test, transform=test_transform)
...
class ISIC_Dataset(Dataset):
    def __init__(self, root_dir, csv_file, transform=None):
        ...
        self.labels = np.eye(self.n_classes)[self.labels.reshape(-1)] ## one_hot labels
        self.transform = transform
